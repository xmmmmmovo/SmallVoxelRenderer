...
bool RayMarchLeaf(vec3 o, vec3 d, out float o_t, out vec3 o_color,
  ...
  float t_min = max(max(2.0f * t_coef.x - t_bias.x, 2.0f * t_coef.y - t_bias.y),
  ...
  while (scale < STACK_SIZE) {
    ...
    if ((idx & step_mask) != 0) {
      ...
      if ((step_mask & 1u) != 0)
        differing_bits |=
            floatBitsToUint(pos.x) ^ floatBitsToUint(pos.x + scale_exp2);
      if ((step_mask & 2u) != 0)
        differing_bits |=
            floatBitsToUint(pos.y) ^ floatBitsToUint(pos.y + scale_exp2);
      if ((step_mask & 4u) != 0)
        differing_bits |=
            floatBitsToUint(pos.z) ^ floatBitsToUint(pos.z + scale_exp2);
  ...
  o_normal = norm;
  o_color = vec3(cur & 0xffu, (cur >> 8u) & 0xffu, (cur >> 16u) & 0xffu) *
            0.00392156862745098f; // (...) / 255.0f
  o_t = t_min;
...
vec3 GenRay() {
  ...
  return normalize(mat3(inverse(uView)) *
                   (inverse(uProjection) * vec4(coord, 1, 1)).xyz);
}
...
void main() {
  ...
  if (uViewType == 2)
    ...
    oFragColor = vec4(hit ? (uViewType == 0 ? color : normal * 0.5f + 0.5f)
                          : vec3(0.8f, 0.8f, 0.8f),
                      1);
}
